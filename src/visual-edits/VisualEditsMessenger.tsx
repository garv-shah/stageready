...
export const CHANNEL = "VISUAL_EDIT_HOVER_v1" as const;
...
const parseVisualEditId = (
  visualEditId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = visualEditId.split(":");
...
export default function HoverReceiver() {
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the visual edit ID from the element to ensure we're working with the right one
    const visualEditId = element.getAttribute("data-visual-edit-id");
    if (!visualEditId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseVisualEditId(visualEditId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same visual edit ID
    const allMatchingElements = document.querySelectorAll(
      `[data-visual-edit-id="${visualEditId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(visualEditId) || {};
    appliedStylesRef.current.set(visualEditId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(visualEditId);
  ...
  const flushImageSrcChange = () => {
    ...
    const visualEditId = imgElement.getAttribute("data-visual-edit-id");
    if (!visualEditId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "VISUAL_EDIT_STYLE_UPDATE") {
      ...
      } else if (e.data?.type === "VISUAL_EDIT_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (parent) {
        ...
        /*
         * Soft-clamp strategy: we respect the parents max size until the
         * users cursor actually travels beyond that limit.  As soon as the
  ...
  useEffect(() => {
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same visual edit ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-visual-edit-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
