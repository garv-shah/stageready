...
function componentTagger(src, map) {
    ...
    try {
        ...
        // 0?? Collect variable declarations first
        const variables = findVariableDeclarations(ast);
        // 1?? Gather local identifiers that reference `next/image`.
        ...
        // 2?? Inject attributes with enhanced semantic context.
        (0, estree_walker_1.walk)(ast, {
            enter(node) {
                var _a;
                ...
                let visualEditId = `${rel}:${line}:${column}`;
                // Enhance the ID with context if we have map information
                if (mapContext) {
                    visualEditId += `@${mapContext.arrayName}`;
                }
                ...
                ms.appendLeft(node.name.end, ` data-visual-edit-id="${visualEditId}" data-visual-edit-name="${semanticName}"`);
                mutated = true;
            },
        });
        if (!mutated)
